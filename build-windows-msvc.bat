...
REM autotools normally writes this header; emit a minimal one so a fresh
REM clone builds without having to run configure first.
if not exist "%REPO%gbxminer-config.h" call :writeconfig
...
for /f "tokens=4 delims=[]" %%V in ('findstr /b "AC_INIT" "%REPO%configure.ac"') do set V=%%V
> "%REPO%gbxminer-config.h" (
  echo /* generated by build-windows-msvc.bat; autotools writes its own */
  echo #define PACKAGE_NAME "gbxminer"
  echo #define PACKAGE_VERSION "!V!"
  echo #define PACKAGE_STRING "gbxminer !V!"
  echo #define HAVE_GETOPT_LONG 1
)
endlocal
echo generated gbxminer-config.h
exit /b 0
