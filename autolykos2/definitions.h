...
/* jsmn.h removed: HTTP polling not used in gbxminer.
 * Provide a minimal stub so json_t in this header compiles.
 * json_t is only used by the HTTP/config layer, never by GPU kernels. */
#ifndef JSMN_H
typedef struct { int start; int end; int type; int size; } jsmntok_t;
#endif
#include <stdio.h>
#include <stdint.h>
#include <stddef.h>
...
/*******************************************************************************
 * GBXminer build parameters — injected by autolykos2/definitions.h
 * The upstream standalone miner defined these per .cu file; we fix them here.
 ******************************************************************************/
#ifndef BLOCK_DIM
#  define BLOCK_DIM          64
#endif
#ifndef NONCES_PER_ITER
#  define NONCES_PER_ITER    0x40000  /* 262144 = 2^18 */
#endif
