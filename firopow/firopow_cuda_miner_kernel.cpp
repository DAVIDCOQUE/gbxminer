/**
 * firopow_cuda_miner_kernel.cpp
 *
 * FiroPow (Firo ProgPoW) kernel management using NVRTC.
 *
 * Design
 * ------
 ...
 * The build still succeeds; FiroPow simply cannot be used.
 */

/* gbxminer-config.h MUST come first — it defines HAVE_NVRTC which gates
 * the macro and type definitions in firopow_cuda_miner_kernel.h.        */
...
#  define NVRTC_SAFE_CALL(call)                                             \
    do {                                                                     \
        nvrtcResult _r = (call);                                             \
        if (NVRTC_SUCCESS != _r) {                                           \
            applog(LOG_ERR, "FiroPow NVRTC error: %s", nvrtcGetErrorString(_r)); \
            return false;                                                    \
        }                                                                    \
    } while (0)
    ...
    uint32_t* state, uint64_t seed, const uint32_t* mix_lane)
{
    ...
    // Full implementation in firopowminer; abbreviated here.
    // TODO: embed full keccak_f800 rounds.
    (void)state; (void)seed; (void)mix_lane;
    return 0;
}

extern "C" __global__ void firopow_search(
    ...
    volatile KawpowResults*    g_output)
{
    ...
    const uint32_t lane_id = threadIdx.x & (FIROPOW_LANES - 1);
    ...
    if (index >= FIROPOW_MAX_RESULTS) return;
...
bool firopow_compile_kernel(uint64_t period, CUmodule* mod_out)
{
...
#ifndef HAVE_NVRTC
    applog(LOG_ERR, "FiroPow: NVRTC not available — cannot JIT-compile kernel");
    return false;
#else
    std::string src = firopow_build_kernel_source(period);
    ...
    if (compile_res != NVRTC_SUCCESS) {
        ...
        applog(LOG_ERR, "FiroPow NVRTC compile error (period %llu):\n%s",
    ...
    if (cu_res != CUDA_SUCCESS) {
        ...
        applog(LOG_ERR, "FiroPow cuModuleLoadData failed (period %llu): %s",
    ...
    *mod_out = mod;
    applog(LOG_INFO, "FiroPow: compiled kernel for period %llu",
...
{
    CUfunction func;
    FIROPOW_CU_SAFE_CALL(cuModuleGetFunction(&func, module, "firopow_search"));
    ...
    FIROPOW_CU_SAFE_CALL(cuLaunchKernel(
        func,
        grid_size,  1, 1,
        block_size, 1, 1,
        0,
    ...
    FIROPOW_CUDA_SAFE_CALL(cudaGetLastError());
}
